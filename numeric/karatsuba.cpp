void convolution_brute(auto a, size_t n, auto b, size_t m, auto ret) {
...
void go_karatsuba(auto a, auto b, auto ret, size_t n) {
	if(n<16) convolution_brute(a, n, b, n, ret); else
	if(n&1) {
		...
		auto ar = a, al = a+n/2, br = b, bl = b+n/2,
...
vector<T> convolution(const vector<T> &a, const vector<T> &b) {
	...
	if(m < n/m) {
		vector<T> ret(n+m-1);
		convolution_brute(begin(a), n, begin(b), m, begin(ret));
		return ret;
	} else {
		vector<T> ret(n*6), b_ex(n);
