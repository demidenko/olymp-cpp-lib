...
struct rmq_range_assign {
	...
	void assign(size_t i, size_t j, const T &val, size_t l, size_t r, size_t v) {
		...
		t[v].first = std::min(t[v*2].first, t[v*2+1].first, cmp);
	}
	
	T min(size_t i, size_t j, size_t l, size_t r, size_t v) const {
		if((i == l && j == r) || t[v].second) return t[v].first;
		if(size_t m = std::midpoint(l, r); j <= m) return min(i, j, l, m, v*2);
		else if(i >= m) return min(i, j, m, r, v*2+1);
		else return std::min(min(i, m, l, m, v*2), min(m, j, m, r, v*2+1), cmp);
	}
};
